...
class BlossomUploadService {
  ...
  BlossomUploadResult _parseUploadResponse(
    ...
    if (response.statusCode == 200 || response.statusCode == 201) {
      ...
      debugPrint(
        '[BlossomAudio] server response (${response.statusCode}): $responseData',
      );
    ...
    try {
      ...
      for (final serverUrl in serverUrls) {
        try {
          ...
          debugPrint(
            '[BlossomAudio] raw result: success=${result.success} '
            'url=${result.url} fallbackUrl=${result.fallbackUrl} '
            'error=${result.errorMessage} statusCode=${result.statusCode}',
          );

          if (result.success) {
            // Use the URL returned by the server (which may include
            // a per-npub subdomain), falling back to canonical format.
            final serverReturnedUrl = result.url;
            final canonicalUrl =
                (serverReturnedUrl != null && serverReturnedUrl.isNotEmpty)
                ? serverReturnedUrl
                : '$_defaultServerUrl/$fileHash';
